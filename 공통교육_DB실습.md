...
2. `attendance` 테이블에서 중복을 제거하기 위해 `crew` 테이블을 만들려고 한다. 어떻게 구성해 볼 수 있을까?
   ...
   - crew_id(int)와 nickname(varchar(50))으로 구성한다. 여기서 키는 crew_id가 된다.
...
```sql
ALTER TABLE crew ADD CONSTRAINT nickname UNIQUE (nickname);
```
...
```sql
-- 어셔가 없으므로 우선 어셔를 crew에 추가합니다.
INSERT INTO crew (nickname) VALUES ("어셔");

SELECT * FROM attendance WHERE crew_id = 13 AND attendance_date = '2025-03-06';
```
...
```sql
INSERT INTO attendance (crew_id, attendance_date, start_time, end_time) VALUES (13, '2025-03-06', '09:31:00','18:01:00');
```
...
```sql
-- 주니가 없으므로 우선 주니를 crew에 추가합니다.
INSERT INTO crew (nickname) VALUES ("주니");
INSERT INTO attendance (crew_id, attendance_date, start_time, end_time) VALUES (14, '2025-03-12', '10:05:00','18:00:00');

UPDATE attendance SET start_time = '10:00:00'
...
```sql
-- 아론이 없으므로 우선 아론을 crew에 추가합니다.
INSERT INTO crew (nickname) VALUES ("아론");
INSERT INTO attendance (crew_id, attendance_date, start_time, end_time) VALUES (15, '2025-03-12', '10:00:00','18:00:00');
...
```sql
SELECT a.attendance_date, a.start_time, a.end_time, c.nickname FROM attendance a 
...
```sql
SELECT DISTINCT a.end_time, c.nickname FROM attendance a 
INNER JOIN crew c ON a.crew_id = c.crew_id
...
```sql
SELECT crew_id, COUNT(*) AS '날짜 수' FROM attendance GROUP BY crew_id;
```

#### 문제 14: 크루별로 등교 기록이 있는(start_time IS NOT NULL) 날짜 수 조회

```sql
SELECT crew_id, COUNT(*) AS '날짜 수' FROM attendance
...
```sql
SELECT attendance_date, COUNT(*) AS '크루 수'
FROM attendance
...
```sql
SELECT crew_id, MIN(start_time) AS '가장 빠른 등교 시간', MAX(start_time) AS '가장 늦은 등교 시간'
FROM attendance
GROUP BY crew_id;
```